...
#define IO_ONLY 5 // reserved for IO access
#define ADDR_A 5  // default address for A B and C
#define ADDR_B 30
#define ADDR_C 55
#define LOOPS 10  // loop number
...
int cache_size = MATRIX_DEFAULT_SIZE*MATRIX_DEFAULT_SIZE; // only for golden answer calculation
int matrix_size = MATRIX_DEFAULT_SIZE;
int loops = LOOPS;
unsigned int process_ID = 1;    // starting from 1, 0 is reserved for invalid request
...
// struct for holding request parameter together
struct Task_Info{
  unsigned int mst_id;
  unsigned int addr;
  unsigned int op;
...
public:
  ...
  std::queue<struct Task_Info> task, ack_task;  // active request and acked request
  std::queue<bool> ack_status, finish_status; // Acknowledge status and finish status

  // track data transfer and bus buy flag
  int target_transfer_counter;
  int actual_transfer_counter;
  bool bus_is_busy;

  sc_in<sc_logic> clk;

  struct Task_Info current_task; // current broadcasting request

  int request_count;    // active reqest number

  SC_HAS_PROCESS(Bus);

  Bus (sc_module_name name) : sc_module(name){
    current_task = Task_Info(0,0,NONE,0); // initilize to invalid task
    bus_is_busy = false;
    request_count = 0;
  ...
  void arbiter(){
    while(true){
      ...
      wait(CLK_PERIOD,SC_NS);
      if(!task.empty()){    // if there are other active request
        if(!bus_is_busy){   // if bus is not busy
          if(current_task.mst_id!=0){   // change request
  ...
  bool WaitForAcknowledge(unsigned int mst_id){
    while(true){
      ...
      if(!ack_task.empty() && !bus_is_busy){
        Task_Info check = ack_task.front();   // check if there are any acked requests
        if (check.mst_id == mst_id){          // if the acked request if myself
          target_transfer_counter = check.len;  // data transfer paramenter save
          actual_transfer_counter = 0;
          if(check.op == C){        // if task is calculation from SW to HW, release bus
            bus_is_busy = false;
          ...
          ack_task.pop(); // !!!
          bool output = ack_status.front();   // push ack result to queue
          ack_status.pop();
          return output;
        }
      }
  ...
  void ReadData(unsigned int &data){
    wait(CLK_PERIOD,SC_NS);
    wait(CLK_PERIOD,SC_NS);
    while(data_buffer.empty()){     // if data not arrived, wait
      wait(CLK_PERIOD,SC_NS);
    }
    data = data_buffer.front(); // read data
    data_buffer.pop();
    actual_transfer_counter++;
    if(actual_transfer_counter==target_transfer_counter){ // check if all data transfer done
      bus_is_busy = false;
    }
  }

  void WriteData(unsigned int data){    // write data to buffer
    wait(CLK_PERIOD,SC_NS);
...
// need to be in a while loop, keep listening
  ...
  void Acknowledge(bool status){
    wait(CLK_PERIOD,SC_NS);
    ack_task.push(current_task);  // push to acked task queue
    ack_status.push(status);      // push ack results
    request_count--;
  ...
  void SendReadData(unsigned int data){   // push read data to buffer
    data_buffer.push(data);
  }

  void ReceiveWriteData(unsigned int &data){    // receive write data from buffer
    while(data_buffer.empty()){     // if data not arrived, wait
      wait(CLK_PERIOD,SC_NS);
    }
    ...
    data_buffer.pop();
    actual_transfer_counter++;
    if(actual_transfer_counter==target_transfer_counter){ // if all data transfer done, release bus
      bus_is_busy = false;
    }
  }
//-------------------------- extra ----------------------
  bool WaitForFinish(){         // only SW call this, to wait for the HW to finish calculation
    while(true){
      wait(CLK_PERIOD,SC_NS);
  ...
  void AckFinish(bool status){      // only HW call this , to tell SW the calculation is finished
  ...
  void Memory_Access(){
    while(true){
      ...
      if(addr>=0 && addr+len-1<=MEMORY_SIZE){ // if memory access in range
        ...
        bus_mem_interface->Acknowledge(true); // ack true
        valid_access = true;
      }else{
        bus_mem_interface->Acknowledge(false); // ack false
      }
      if(valid_access){   // access memory according to request (read or write)
        for (int i = 0; i<len; i++){
          if(op==R){
  ...
  void function(){
    while(true){
      ...
      while(true){
        wait(CLK_PERIOD,SC_NS);
        io_access = false;
        slave->Listen(addr,op,len); // listen for request from SW
        if(op==C){
      ...
      if(io_access == true){
        ...
        master->Request(ID,command_addr,R,4);   // request to access command from IO only section
      ...
      // reset C with burst transaction
      if(memory_access_C){
        ID = process_ID;
        process_ID++;
      ...
      if(calculate){
        ...
        master->AckFinish(true);    // tell SW the finish status
      }else{
        cout << "Hardware: Task Failed"<< endl;
        master->AckFinish(false); // tell SW the finish status
      }
    }
  }
};
...
public:
  ...
  void With_Hardware(){
    for (int i=0; i<loops; i++){
      cout << "==================LOOP "<< i <<"==================" << endl;
      // write command to IO only section
      ID = process_ID;
      process_ID++;
